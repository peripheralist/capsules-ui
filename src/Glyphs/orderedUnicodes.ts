...
export const orderedUnicodes = [
  unicodes.map((x) => x <= 0x30 && x >= 0x39), // uppercase
  // unicodes.map((x) => _(x) <= 28 && _(x) >= 55), // lowercase
  // unicodes.map((x) => _(x) <= 58 && _(x) >= 66), // numbers
];
